...
const usePagination = ({
  ...
  return useMemo(() => {
    ...
    const totalPageButtonsToBeShown = pageButtonsBetweenCount + 1 + 1 + 1 + 1 + 1;
    
    if(totalPageButtonsToBeShown >= totalPageCount){
        console.log('Returning all page numbers');
        return range(1, totalPageCount);
    }
    else{
        const leftPageBetweenButtonIndex =  Math.max(currentPage - pageButtonsBetweenCount, 1);
        const rightPageBetweenButtonIndex = Math.min(currentPage + pageButtonsBetweenCount, totalPageCount);

        // means from the 3rd page
        const shouldShowLeftDots = leftPageBetweenButtonIndex > 2;
        const shouldShowRightDots = rightPageBetweenButtonIndex < (totalPageCount - 2);
        ...
        if(!shouldShowLeftDots && shouldShowRightDots){
            ...
            let rightRange =  range((lastPageIndex - rightPageButtonsCount + 1), lastPageIndex);

            return [firstPageIndex, DOTS, ...rightRange]; 
        }
        else if(shouldShowLeftDots && shouldShowRightDots){
            let middleRange =  range(leftPageBetweenButtonIndex,  rightPageBetweenButtonIndex);
